...
bool canPlaceDigit(int grid[][9], int n, int i, int j, int d) {
	...
	for (int k = 0; k < n; k++) {
		if (grid[i][k] == d || grid[k][j] == d) {
			// you've found digit 'd' is the ith row or jth column therefore it cannot be assigned to the (i, j)th cell
			return false;
		}
	}

	// check if digit 'd' is present in the subgrid of (i, j)th cell
	...
	for (int x = sx; x < sx + rn; x++) {
		for (int y = sy; y < sy + rn; y++) {
			if (grid[x][y] == d) {
				// you've found digit 'd' in the subgrid of (i, j)th cell therefore it cannot be assigned to the (i, j)th cell
				return false;
			}
		}
	}

	// you can assign digit 'd' to the (i, j)th cell
	return true;
