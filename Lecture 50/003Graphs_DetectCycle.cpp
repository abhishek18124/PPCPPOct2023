	...
	bool dfsHelper(T source, unordered_set<T>& visited, T parent) {
		...
		for (T neighbour : neighbourList) {
			if (visited.find(neighbour) == visited.end()) {
				...
				// is the edge from source(u) to neighbour(v) a backedge ?

				// yes, if neighbour(v) is not the parent of source(u) in the dfs-tree

				if (neighbour != parent) {

					// you've found a back-edge therefore you've a cycle
