		...
		while (!q.empty()) {
			...
			for (T ngb : neighbourList) {
				if (colorMap.find(ngb) == colorMap.end()) {
					// ngn is not yet visited / colored
					colorMap[ngb] = 1 - colorMap[front];
					q.push(ngb);
				} else {
					// ngb is already colored
					if (colorMap[front] == colorMap[ngb]) {
						// adj. nodes have same color therefore this component is not bipartite
						return false;
					}
				}
			}
		...
		for (pair<T, list<T>> p : neighbourMap) {
			...
			if (colorMap.find(vertexLabel) == colorMap.end()) {
				...
				if (bfsHelper(vertexLabel, colorMap) == false) {
					// component is not bipartite therefor the graph is not bipartite
					flag = false;
					break;
				}
			}
