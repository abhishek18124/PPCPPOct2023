...
void generateCombinations(int t, vector<int> c, vector<int>& comb, int i) {
	...
	for (int j = i; j < c.size(); j++) {
		...
		if (c[j] <= t) {
			...
			comb.push_back(c[j]);
			// generateCombinations(t - c[j], c, comb, j + 1); // this make sures that each candidate is used only once
			generateCombinations(t - c[j], c, comb, j); // this make sures that each candidate is unlimited no. of times
			comb.pop_back();
